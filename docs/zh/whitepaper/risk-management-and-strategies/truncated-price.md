...
在 Likwid v2.2 中，**Truncated Price（截断价格参考）** 是一种从池子储备推导出的、受边界约束的内部价格参考。协议同时维护实时的 pair reserve 和 truncated reserve。随着时间推移，truncated reserve 会以 `priceMoveSpeedPPM` 限制的速度逐步向实时 pair 状态靠拢，而不是立即追随每一次短期储备冲击。
